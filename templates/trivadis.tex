...
$endif$
\usepackage{xcolor}
\definecolor{default-linkcolor}{HTML}{9600FF}
\definecolor{default-filecolor}{HTML}{9600FF}
\definecolor{default-citecolor}{HTML}{4077C0}
\definecolor{default-urlcolor}{HTML}{9600FF}
...
$endif$
  breaklinks=true,
  pdfcreator={LaTeX via pandoc with the Trivadis template (based on Eisvogel)}}
...
% Customize unorderet list
\definecolor{default-bulletcolor}{HTML}{000000}
\newcommand{\localtextbulletone}{\textcolor{$if(bulletcolor)$$bulletcolor$$else$default-bulletcolor$endif$}{\raisebox{.45ex}{\rule{.9ex}{.9ex}}}}
...
\newfontfamily\tvdmfamily{Montserrat}
\newfontfamily\tvdofamily{Open Sans Light}
\addtokomafont{title}{\sffamily\MakeUppercase}
\addtokomafont{subtitle}{\ttfamily\bfseries}

% modify section header fonts
\makeatletter
\renewcommand\sectionlinesformat[4]{\ifstr{#1}{section}{\@hangfrom{\sffamily\hskip #2#3}{\sffamily\MakeUppercase{#4}}}{\@hangfrom{\normalfont\bfseries\hskip #2#3}{\normalfont\bfseries\MakeUppercase{#4}}}}
\renewcommand\sectioncatchphraseformat[4]{\ifstr{#1}{section}{\@hangfrom{\sffamily\hskip #2#3}{\sffamily\MakeUppercase{#4}}}{\@hangfrom{\normalfont\bfseries\hskip #2#3}{\normalfont\bfseries\MakeUppercase{#4}}}}
\makeatother
...
\providecommand{\subtitle}[1]{% add subtitle to \maketitle
  \apptocmd{\@title}{\par {\MakeUppercase\large #1 \par}}{}{}
}
\makeatother
$endif$
\subtitle{$subtitle$}
...
$endif$
$if(titlepage-logo)$
\titlepage-logo{\includegraphics{$titlepage-logo$}}
$endif$
$endif$
...
%
\definecolor{listing-background}{HTML}{F2F2F2}
\definecolor{listing-rule}{HTML}{BFBFBF}
...
\lstdefinestyle{trivadis_listing_style}{
  language         = java,
...
\lstset{style=trivadis_listing_style}
...
$else$
\usepackage{lastpage}
\newcommand*{\TVDLogo}{$if(footer-logo)$\includegraphics[width=0.12\textwidth,valign=c]{$footer-logo$}$endif$}
\usepackage[headsepline,footsepline]{scrlayer-scrpage}

\newpairofpagestyles{trivadis-header-footer}{
  \clearpairofpagestyles
  \ihead[$if(header-right)$$header-right$$else$  $endif$]{$if(header-left)$$header-left$$else$$endif$}
  \chead[$if(header-center)$$header-center$$else$$endif$]{$if(header-center)$$header-center$$else$$endif$}
  \ohead[$if(header-left)$$header-left$$else$$endif$]{$if(header-right)$$header-right$$else$$endif$}
  \ifoot[$if(footer-right)$$footer-right$$else$\textbf{TRIVADIS.COM}$endif$]{$if(footer-left)$$footer-left$$else$\textbf{TRIVADIS.COM}$endif$}
  \cfoot[$if(footer-center)$$footer-center$$else$\thepage\ / \pageref*{LastPage}$endif$]{$if(footer-center)$$footer-center$$else$\thepage\ / \pageref*{LastPage}$endif$}
  \ofoot[$if(footer-left)$$footer-left$$else$\TVDLogo$endif$]{$if(footer-right)$$footer-right$$else$\TVDLogo$endif$}
  \addtokomafont{pageheadfoot}{\upshape}
}
\pagestyle{trivadis-header-footer}
$if(page-background)$
\backgroundsetup{
scale=1,
color=black,
...
$endif$
  ...
  $if(titlepage-logo)$
    \begin{flushright}
      \includegraphics[width=5.12cm, right]{$titlepage-logo$}
    \end{flushright}
  $endif$
...
\begin{flushright}
  \setstretch{2}
  \vfill
  \noindent {\huge \MakeUppercase{\textbf{\textsf{$title$}}}}\\
  $if(subtitle)$
  \vskip 1em
    {\Large \MakeUppercase{\textsf{$subtitle$}}}\\
  $endif$
  \bigskip
  \textbf{$if(date)$$date$$else$\today$endif$, Version $if(tvddocversion)$$tvddocversion$$else$1.0$endif$ }\\
  \vfill
\end{flushright}
}
$else$
...
\begin{flushright}
  \setstretch{1.4}
  \vfill
  \noindent {\huge \textbf{\MakeUppercase{\textsf{$title$}}}}\\
  $if(subtitle)$
  \vskip 1em
    {\Large \MakeUppercase{\textsf{$subtitle$}}}\\
  $endif$
...
$if(titlepage-background)$
  ...
  %% TVD Contact
  \begin{flushright}
    $if(tvdcontact)$ \textit{$tvdcontact$\\} $else$ \author{$for(author)$$author$$sep$ \and $endfor$} $endif$
...
$else$
  ...
  \begin{flushright}
    ...
    $if(tvdlocplz)$ \textit{$tvdlocplz$}  $else$ \textit{8152} $endif$
    $if(tvdloccity)$ \textit{$tvdloccity$}  $else$ \textit{Glattbrugg} $endif$
    $if(tvdloccountry)$ \textit{$tvdloccountry$} $endif$
    \par
  \end{flushright}
